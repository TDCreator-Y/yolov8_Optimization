...
yolo export model=yolov8n_baseline.pt imgsz=640 format=mnn
```
...
yolo export model=yolov8n_baseline.pt format=onnx
./MNN/build/MNNConvert -f ONNX --modelFile yolov8n_baseline.onnx --MNNModel yolov8n_baseline.mnn --bizCode biz
```
...
```bash
yolo predict model=yolov8n_baseline.mnn source=bus.jpg
```
...
```bash
./build/main yolov8n_baseline.mnn bus.jpg
```
...
```bash
./build/main_interpreter yolov8n_baseline.mnn bus.jpg
```
