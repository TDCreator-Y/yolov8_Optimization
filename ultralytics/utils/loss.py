...
def wIoU(pred_boxes, target_boxes, alpha=0.5, beta=2.0, xywh=True, eps=1e-7, clamp_val=4.0, warmup=False):
    ...
    Args:
        ...
        target_boxes: [N,4] tensor, target boxes
        alpha, beta: weight hyperparameters
        xywh: if True, input boxes are in xywh format, else xyxy
        eps: small number to avoid division by zero or log(0)
        clamp_val: max value for center_dist and wh_ratio to prevent gradient explosion
        warmup: if True, use normal IoU loss for warmup
    ...
    # 权重
    weight = torch.exp(-alpha * center_dist - beta * wh_ratio)
